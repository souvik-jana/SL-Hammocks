...
Dependencies ([glafic](https://github.com/oguri/glafic2), [Colossus](https://bdiemer.bitbucket.io/colossus/), numpy, scipy, joblib, etc.) are managed with [pixi](https://pixi.sh) (Linux x86_64).

```bash
curl -fsSL https://pixi.sh/install.sh | bash   # install pixi
pixi install                                   # create environment
pixi run setup                                 # build glafic
```

Run `pixi run install-glafic` again after recreating the environment. Use `pixi run test` to run the test suite.

## Examples

The full (1 realization) LSST quasar lens mock catalog is generated by
```
pixi run python gen_mock_halo.py --area=20000.0 --ilim=23.3 --zlmax=3.0 --source=qso --prefix=qso_mock --solver=glafic --nworker=1
```
The full (1 realization) LSST supernova lens mock catalog is generated by
```
pixi run python gen_mock_halo.py --area=50000.0 --ilim=22.6 --zlmax=2.0 --source=sn --prefix=sne_mock --solver=glafic --nworker=1
```
